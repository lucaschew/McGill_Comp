...
def read_and_process_complaints(file_path, start_date, end_date):
    ...
    df = pd.read_csv(file_path, header=None, usecols=[1, 5, 25], names=['creation_date', 'complaint_type', 'borough'])
    ...
    grouped = filtered_df.groupby(['borough', 'complaint_type']).size().reset_index(name='count')
